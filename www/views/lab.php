	<section>
		<article>
			<h2>実験的なもの</h2>
		</article>
	</section>
	<section>
		<article>
			<h3><a href="/app/pixelword_printer" target="_blank" >PixelWord Printer</a></h3>
			<p>8bitな文字入力アプリ</p>
			<p>その時代の配置を意識した濁点半濁点の位置の設定、画像として出力もできます。</p>
			<p class="text_center"><img src="/img/lablist/encode_mailaddr_01.png" /></p>
		</article>
	</section>
	<section>
		<article>
			<h3><a href="/app/encode_mailaddr" target="_blank" >encode mailaddr</a></h3>
			<p>メールなどをエンコードしてリンクを生成</p>
			<p>aタグのhrefに入れてお使い下さい</p>
			<p class="text_center"><img src="/img/lablist/encode_mailaddr_01.png" /></p>
		</article>
	</section>
	<section>
		<article>
			<h3><a href="/app/thumbtri" target="_blank" >サムネトリマー「さむとり」</a></h3>
			<!--<h3><a href="http://jsdo.it/bitchunk/qGq6/fullscreen" target="_blank" >サムネトリマー「さむとり」</a></h3>-->
			<p>読み込んだ画像から範囲を選択してサムネイルを生成します。</p>
			<p class="text_center"><img src="/img/lablist/thumbtri_01.jpg" /></p>
		</article>
	</section>
